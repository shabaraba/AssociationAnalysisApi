...
def handler(event, context):
    ...
    df_rules['antecedents_len'] = df_rules['antecedents'].apply(len)
    df_rules['consequents_len'] = df_rules['consequents'].apply(len)
    print (df_rules)
    print('analyze result::')
    print(df_rules.loc[:,["antecedents","consequents","support","confidence","lift", "antecedents_len", "consequents_len"]])

    condition = request.get("condition")
    results = df_rules[
        (df_rules['confidence'] > float(condition.get("confidence", 0.2))) &  # 信頼度
        (df_rules['lift'] > float(condition.get("lift", 1.0)))  # リフト値
        & (df_rules['antecedents_len'] == 1)
        & (df_rules['consequents_len'] == 1)
    ]
    print('filter result::')
